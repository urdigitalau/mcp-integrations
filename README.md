...
**WordPress** (`packages/wordpress`) — `wp_list_posts`, `wp_get_post`,
`wp_create_post`, `wp_update_post`, `wp_delete_post`, `wp_list_pages`,
`wp_upload_media`, `wp_search`. New posts default to `draft` status so
