...
export function requireEnv(name: string): string {
  ...
  if (!value || value.trim().length === 0) {
    throw new Error(
      `Missing required environment variable: ${name}. Set it in your MCP client config (e.g. claude_desktop_config.json "env" block) or a .env file loaded before start.`
    );
  }
  return value;
}
